...
local ArticleItem = InputContainer:extend{
    ...
    hold_callback = nil,  -- function(article) called on hold
...
function ArticleItem:init()
    ...
    local is_unread = not self.article.read
    local title_face   = Font:getFace(
        is_unread and "smallinfofontbold" or "smallinfofont",
        is_unread and (sz + 1) or sz)
    ...
    local rt = TextUtil.readingTime(TextUtil.articlePlainText(self.article))
    if rt then table.insert(meta_parts, rt) end
    if self.article.starred then
        table.insert(meta_parts, 1, "★")
    end
